...
describe('{{ghost_head}} helper', function () {
    ...
    describe('includes tinybird tracker script when config is set', function () {
        ...
        it('sets tb_gift_link to the token on a verified gift read', async function () {
            ...
            // `_giftLink` on the template data frame (templateOptions.data, the
            // same frame ghost_foot reads for the toast) — NOT on the data root.
            // The tracker reads `options.data._giftLink` and forwards it so
            // analytics can segment gift traffic and count per-link usage. Inject
            // it the same way here (via templateOptions, not locals) so this
            // guards the real production data path.
            const rendered = (await ghost_head(testUtils.createHbsResponse({
                renderObject: {post: posts[10]},
                templateOptions: {_giftLink: 'gift_token_abc'},
                locals: {
                    relativeUrl: '/post/',
                    context: ['post'],
                    safeVersion: '4.3'
